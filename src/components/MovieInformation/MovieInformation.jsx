...
import { Modal, Typography, Button, ButtonGroup, Grid, Box, CircularProgress, useMediaQuery,ListItemIcon, Rating } from '@mui/material';
        ...
        <Grid item className={classes.genresContainer}>
          {data?.genres?.map((genre, i) => (
            <Link key={genre.name} className={classes.links} to="/" onClick={() => dispatch(selectGenreOrCategory(genre.id))}>
                <img src={genreIcons[genre.name.toLowerCase()]} className={classes.genreImage} height={30}></img>
        ...
        <Grid item container spacing={2}>
          {data && data.credits?.cast?.map((character, i) => (
            ...
            character.profile_path && <Grid key={i} item xs={4} md={2} component={Link} to={`/actors/${character.id}`} style={{textDecoration: "none"}}>
              <img className={classes.castImage} src={`https://image.tmdb.org/t/p/w500/${character.profile_path}`} alt={character.name}></img>
          ...
          <div className={classes.buttonsContainer}>
            ...
            <Grid item xs={12} sm={6} className={classes.buttonsContainer}>
              <ButtonGroup size="medium" variant='outlined'>
                <Button onClick={add2Favs} endIcon={isMovieFav ? <FavoriteBorderOutlined/> : <Favorite/>}>{isMovieFav ? 'Unfavourite' : 'favourite' }</Button>
                <Button onClick={add2WatchList} endIcon={isMovieWatched ? <Remove/> : <plusOne/>}>Watchlist</Button>
                <Button endIcon={ <ArrowBack/>} sx={{borderColor: 'primary.main'}}>
                  <Typography component={Link} style={{textDecoration: 'none'}}  to="/" color='inherit' variant='subtitle2'>Back </Typography>
                </Button>
